...
#
# **Requirements:** `pip install -e ".[dev]"`. No `pycocotools` dependency - `oodkit.contrib.coco` parses the JSON directly.
#
...
HEAD_EPOCHS = 3
BATCH_SIZE = 64
NUM_WORKERS = 0  # Use 0 in notebooks to avoid multiprocessing cleanup noise.
PIN_MEMORY = True
...
#   a single high-scoring chip; `topk_mean` is a cheap compromise. Future work
#   will explore geometry- / context-aware pooling.
